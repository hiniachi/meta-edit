...
- The user asks to add an "uncertainty markers" / "hedging vocabulary" /
  "ambiguity tags" section to the project's `CLAUDE.md`.
- The user invokes `/ambiguity-markers`.
- During a CLAUDE.md audit when the project's prose-hedging convention is
...
1. **Read** the current project's `CLAUDE.md` (always start here).
2. **Look at the source tree casually** — `ls src/`, walk a level or two, scan
   ...
   no formal detection.
3. **Judge idempotency semantically** — does the existing `CLAUDE.md` already
   document either of the two axes (§1), in any form? The judgment is
...
7. **Compose the proposed CLAUDE.md section** using the §4 skeleton, with
...
10. **Never touch source files, tests, or non-`CLAUDE.md` docs**. The skill
    writes one file: the project's top-level `CLAUDE.md`.
...
### Markdown (`*.md`, including `CLAUDE.md`, `README.md`, design docs, planning docs)
...
## 4. The CLAUDE.md section the skill writes
...
- **Machine-readable metadata** — the markers are greppable strings, not
  parsed annotations.
- **Editing source files, tests, or non-`CLAUDE.md` docs** — the skill
...
- **Per-user `~/.claude/rules/*.md`** — conventions are project-shaped, not
  user-shaped. This writes to the project `CLAUDE.md`.
...
- Re-read the current `CLAUDE.md` from scratch.
