...
` || tc === ">" || tc === "<") {
    ...
    if (opts.cwd && target.length > 0) {
      ...
      const rel = path3.relative(protectedRoot, absolute);
      if (rel.length > 0 && isProtectedPath(rel, { repoRoot: protectedRoot })) {
        return true;
      }
    }
    i = j;
...
function discoverRepoRoot(start) {
  const resolvedStart = path4.resolve(start);
  const realStart = realpathOfDeepestExisting(resolvedStart) ?? resolvedStart;
  const realTmp = realpathOfDeepestExisting(os.tmpdir()) ?? path4.resolve(os.tmpdir());
  ...
  return realpathOfDeepestExisting(base) ?? path4.resolve(base);
}
function hasRepoMarker(dir, realStart, realTmp) {
  const markers = [path4.join(dir, ".git"), path4.join(dir, ".jj")];
  if (!markers.some((marker) => fs3.existsSync(marker))) {
    return false;
  }
  const realDir = realpathOfDeepestExisting(dir) ?? path4.resolve(dir);
  if (realDir === realTmp && realStart !== realTmp) {
    const onlyEmptyTempMarkers = markers.every((marker) => {
      if (!fs3.existsSync(marker))
        return true;
      try {
        const stat = fs3.statSync(marker);
        return stat.isDirectory() && fs3.readdirSync(marker).length === 0;
      } catch {
        return false;
      }
    });
    if (onlyEmptyTempMarkers)
      return false;
  }
  return true;
}
...
function canonicalizeRepoRelative(inputPath, repoRoot) {
  const resolved = path4.isAbsolute(inputPath) ? path4.normalize(inputPath) : path4.resolve(repoRoot, inputPath);
  const realRoot = realpathOfDeepestExisting(path4.resolve(repoRoot)) ?? path4.resolve(repoRoot);
  ...
  let rel;
  try {
    rel = normalizeRepoRelative(path4.relative(realRoot, realResolved));
  } catch (e) {
    return {
      ok: false,
      code: "uncanonicalizable",
...
function repoIsValid(dir) {
  ...
  const found = sentinels.some((s) => fs4.existsSync(path5.join(dir, s)));
  if (found)
    return { ok: true };
  return {
    ok: false,
...
function checkPathSafety(p, repoRoot) {
  ...
  const absInput = path6.resolve(repoRoot, p);
  try {
    const lst = fs5.lstatSync(absInput);
    if (lst.isSymbolicLink()) {
      const linkTarget = fs5.readlinkSync(absInput);
      const resolvedTarget = path6.resolve(path6.dirname(absInput), linkTarget);
      const targetRel = path6.relative(repoRoot, resolvedTarget);
...
function computeBeforeSha256(canonical, repoRoot, fieldLabel) {
  const absolute = path6.join(repoRoot, canonical);
  let onDisk = null;
  try {
...
class EditLog {
  ...
  constructor(repoRoot) {
    this.statePath = path7.join(repoRoot, ".meta-edit", "state");
    this.logPath = path7.join(this.statePath, "edits.jsonl");
  }
  get filePath() {
    return this.logPath;
  }
...
`;
  ...
  withFileLock(fn) {
    const lockPath = path7.join(this.statePath, ".lock");
  ...
  readCounterFile(key) {
    const counterPath = path7.join(this.statePath, "counter.json");
    let text;
    try {
...
function ensureNoSymlinkOnPath(maybeRelativeDir) {
  const absDir = path7.resolve(maybeRelativeDir);
  const segments = absDir.split(path7.sep).filter((s) => s.length > 0);
  let cur = path7.sep;
  for (const seg of segments) {
    cur = path7.join(cur, seg);
    let stat;
    try {
...
class GrantsStoreImpl {
  grantsDir;
  constructor(repoRoot) {
    this.grantsDir = path8.join(repoRoot, ".meta-edit", "state", "grants");
  }
  mutexKey(token_id) {
    return path8.resolve(this.grantPath(token_id));
  }
  async ensureDir() {
    await fs7.mkdir(this.grantsDir, { recursive: true, mode: 448 });
  }
  grantPath(token_id) {
    return path8.join(this.grantsDir, `${token_id}.json`);
  }
  async issue(args) {
  ...
  async findActiveBindingForFile(canonicalFile, opts) {
    ...
    for (const name of names) {
      if (!name.endsWith(".json"))
        continue;
      const filePath = path8.join(this.grantsDir, name);
      let text;
      try {
        text = await fs7.readFile(filePath, "utf8");
      } catch (e) {
...
// src/codex/apply-patch-policy.ts
...
// src/codex/apply-patch-policy.ts
function extractApplyPatchTargets(patch) {
  ...
  for (const line of lines.slice(1, endIndex)) {
    if (line.startsWith("*** Move to:")) {
      return {
        ok: false,
        error: "apply_patch move/rename patches are not supported by meta-edit yet; declare and edit the final file explicitly"
      };
    }
...
async function evaluateCodexApplyPatch(args) {
  ...
  for (const target of parsed.targets) {
    ...
    if (!pathCheck.ok) {
      return {
        decision: "deny",
        reason: `[meta-edit:path-mismatch] apply_patch target "${target.path}" is not safe: ${pathCheck.error}`
      };
    }
    ...
    if (!diskRead.ok) {
      return {
        decision: "deny",
        reason: `[meta-edit:unreadable] could not read "${canonical}" before apply_patch preflight ` + `(${diskRead.error}); re-read and re-issue a typed_edit declaration.`
      };
    }
...
` + `That would skip the intended classification step. The correct next move is to call a typed edit_* MCP tool first, then retry the patch.`
    ...
    if (match.binding.before_sha256 !== diskSha) {
      return {
        decision: "deny",
        reason: `[meta-edit:stale] apply_patch preflight found disk drift for "${canonical}" ` + `(declared before_sha256=${shortHash(match.binding.before_sha256)}, actual ${shortHash(diskSha)}). ` + `Re-read the file and issue a fresh typed_edit declaration before retrying.`
      };
    }
  ...
  for (const item of preflight) {
    const consumeRes = await args.grants.consume(item.match.grant.token_id, item.canonical);
    if (!consumeRes.consumed) {
      const err = consumeRes.error ?? "unknown error";
      if (consumedCount > 0) {
        const reason = `[meta-edit:partial-consume] apply_patch preflight succeeded, but consuming "${item.canonical}" failed after ${consumedCount} earlier binding(s) were already consumed: ${err}. ` + "The patch is allowed to proceed (warn, not a block) to avoid burning the already-consumed grants; the uncovered target was not separately consumed — re-check the edit log afterward.";
        return {
          decision: "warn",
          reason,
...
`) } : {}
      ...
      return {
        decision: "deny",
        reason: `[meta-edit:consume-failed] apply_patch preflight succeeded but consuming "${item.canonical}" failed: ${err}. ` + "Re-declare with a typed edit_* MCP tool before retrying."
      };
    }
    consumedCount++;
    const consumed = {
    ...
    try {
      ...
      process.stderr.write(`[meta-edit] WARN: failed to append consumed record for ${item.match.grant.edit_id}: ${e.message}
`);
    }
    const declaration = item.match.grant.declaration;
    if (declaration !== undefined) {
      contexts.push(buildReminderContext({
        phase: "write_allowed",
        kind: declaration.kind,
        ...declaration.target !== undefined ? { target: declaration.target } : {},
        provenance: declaration.provenance,
        ...declaration.execution_state !== undefined ? { executionState: declaration.execution_state } : {},
...
async function readFileForBinding(repoRoot, canonical) {
  try {
    return {
      ok: true,
      content: await fs8.readFile(path9.join(repoRoot, canonical), "utf8")
    };
  } catch (e) {
    const err = e;
    if (err.code === "ENOENT") {
...
function parseCodexHookPayload(payload) {
  if (!isRecord(payload)) {
    return { ok: false, error: "Codex hook payload must be a JSON object" };
  }
  const hookEventName = stringField(payload, "hook_event_name") ?? stringField(payload, "hookEventName");
...
function renderCodexHookResponse(decision) {
  if (decision.decision === "allow") {
    return decision.additionalContext !== undefined ? { additional_context: decision.additionalContext } : {};
  }
  if (decision.decision === "warn") {
    const parts = [decision.reason, decision.additionalContext].filter((part) => typeof part === "string" && part.length > 0);
    return parts.length > 0 ? { additional_context: parts.join(`

`) } : {};
  }
  const out = { decision: decision.decision };
  if (decision.decision === "deny")
    out.decision = "block";
  out.reason = typeof decision.reason === "string" && decision.reason.length > 0 ? decision.reason : "denied by meta-edit";
...
// src/codex/hook-entrypoint.ts
var UNSUPPORTED_CODEX_RAW_WRITE_TOOLS = new Set([
  "Edit",
  "Write",
  "MultiEdit",
  "NotebookEdit"
]);
async function handleCodexHookPayload(payload, options = {}) {
  ...
  if (toolName === "Bash") {
    ...
    return renderCodexHookResponse(evaluateBashCommand(command, {
  ...
  if (UNSUPPORTED_CODEX_RAW_WRITE_TOOLS.has(toolName)) {
    return renderCodexHookResponse({
      decision: "deny",
      reason: `Codex ${toolName} raw write payloads are unsupported by meta-edit; use apply_patch after a typed edit declaration`
    });
  }
  if (toolName === "apply_patch") {
    ...
    return renderCodexHookResponse(await evaluateCodexApplyPatch({
      patch: patch.patch,
      repoRoot,
      grants,
      log,
...
function handleSessionStart(payload, repoRoot) {
  const sessionId = sessionIdFromPayload(payload);
  if (sessionId === null)
    return { decision: "allow" };
  const source = sessionSourceFromPayload(payload);
  const markerPath = path10.join(repoRoot, ".meta-edit", "state", "sessions", `${sessionMarkerKey(sessionId, source)}.json`);
...
function extractPatchText(input) {
  ...
  if (fromFileChanges !== null)
    return fromFileChanges;
  return {
    ok: false,
    error: "Codex apply_patch payload did not include a patch string or fileChanges map"
  };
}
function patchFromFileChanges(value) {
  if (value === undefined)
    return null;
  ...
  for (const [file, change] of Object.entries(value)) {
    if (file.length === 0) {
      return {
        ok: false,
    ...
    if (/[\r\n]/u.test(file)) {
      return {
        ok: false,
        error: "Codex apply_patch fileChanges path contains CR/LF or newline header injection"
      };
    }
    if (typeof change !== "object" || change === null || Array.isArray(change)) {
      return {
        ok: false,
        error: `Codex apply_patch fileChanges entry for "${file}" must be an object`
      };
    }
    const record = change;
    ...
    switch (record["type"]) {
      ...
      default:
        return {
          ok: false,
          error: `Codex apply_patch fileChanges entry for "${file}" has unsupported type`
        };
    }
  }
...
function sessionSourceFromPayload(payload) {
  if (typeof payload !== "object" || payload === null || Array.isArray(payload)) {
    return "startup";
  }
...
function sessionMarkerKey(sessionId, source) {
  const sourceKey = source === "clear" || source === "compact" ? source : "session";
  return `${sourceKey}-${crypto4.createHash("sha256").update(sessionId).digest("hex")}`;
}
function claimOnboardingMarker(markerPath, sessionId) {
  try {
    fs9.mkdirSync(path10.dirname(markerPath), { recursive: true });
  } catch {
    return false;
  }
  try {
    fs9.writeFileSync(markerPath, JSON.stringify({
...
main().then((code) => process.exit(code), (err) => {
  process.stdout.write(JSON.stringify(renderCodexHookResponse({
    decision: "deny",
    reason: `meta-edit Codex hook crashed: ${err.message}`
  })));
  process.exit(0);
});

//# debugId=1587E412B943649664756E2164756E21
